...
class TaskModelService:
    ...
    @staticmethod
    def update(
        task: TaskModel, updates: TaskModelInterface, app: Flask, db: SQLAlchemy
    ) -> TaskModel:
        """
        This method updates the current task in the DB.
        If the task does not exit in the DB it will save it.
        """
        if not TaskModelService.retrieve_by_task_id(task.task_id, app):
    ...
    def delete(task_id: int, app: Flask, db: SQLAlchemy) -> int:
        ...
        """
        task: TaskModel = TaskModelService.retrieve_by_task_id(task_id, app)
        if task:

            tasks: TaskModel = TaskModelService.retrieve_tasks_by_parent_task_id(
                task_id, app
            )
            for t in tasks:
    ...
    @staticmethod
    def retrieve_tasks_by_parent_task_id(
        parent_task_id: int, app: Flask
    ) -> List[TaskModel]:
        """
    ...
    @staticmethod
    def retrieve_tasks_by_parent_event_id(
...
class TasksListModelService:
    ...
    @staticmethod
    def create(
        list_attrs: TasksListModelInterface, app: Flask, db: SQLAlchemy
    ) -> TasksListModel:
        """
        ...
        if app.config["DEBUG"] or app.config["TESTING"]:
            query = """
                    INSERT INTO TasksLists (list_title, user_id) VALUES (?, ?);
                    """
            results = DBMan.execute_sql_query(
                app, query, (new_list.list_title, new_list.user_id)
            )
            return new_list.update(dict(list_id=results[0]))
        else:
    ...
    @staticmethod
    def update(
        _list: TasksListModel,
        updates: TasksListModelInterface,
        app: Flask,
        db: SQLAlchemy,
    ) -> TasksListModel:
        """
        ...
        if app.config["DEBUG"] or app.config["TESTING"]:
            ...
            DBMan.execute_sql_query(app, query, (_list.list_title, _list.list_id))
        else:
            db.session.commit()
    ...
    def delete(list_id: int, app: Flask, db: SQLAlchemy) -> int:
        ...
        tasks: List[TaskModel] = TaskModelService.retrieve_tasks_by_parent_task_id(
            list_id, app
        )
        for task in tasks:
    ...
    def retrieve_by_list_id(list_id: int, app: Flask) -> TasksListModel:
        ...
        if app.config["DEBUG"] or app.config["TESTING"]:
            ...
            for row in rows_n_rowid[1]:
                return new_list.update(dict(list_id=row[0], list_title=row[1]))
    ...
    def retrieve_lists_by_user_id(user_id: int, app: Flask) -> List[TasksListModel]:
        ...
        if app.config["DEBUG"] or app.config["TESTING"]:
            ...
            for row in rows_n_rowid[1]:
                new_list: TasksListModel = TasksListModel()
                lists.append(new_list.update(dict(list_id=row[0], list_title=row[1])))
            return lists
