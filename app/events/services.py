...
class EventModelService:
    ...
    def json(event: EventModel, app: Flask):
        ...
        return {
            ...
            "time_slots": [
                EventsTimeSlotModelService.json(time_slot)
                for time_slot in EventsTimeSlotModelService.retrieve_slots_by_event_id(
                    event.event_id, app
                )
            ],
        }
    ...
    @staticmethod
    def create(
        event_attrs: EventModelInterface, app: Flask, db: SQLAlchemy
    ) -> EventModel:
        """
    ...
    @staticmethod
    def update(
        event: EventModel, updates: EventModelInterface, app: Flask, db: SQLAlchemy
    ) -> EventModel:
        """
        ...
        if not EventModelService.retrieve_by_event_id(event.event_id, app):
    ...
    def delete(event_id: int, app: Flask, db: SQLAlchemy) -> int:
        ...
        """
        event: EventModel = EventModelService.retrieve_by_event_id(event_id, app)
        if event:

            events = EventModelService.retrieve_events_by_parent_id(event_id, app)
            for e in events:

                EventModelService.delete(e.event_id, app, db)

            tasks: TaskModel = TaskModelService.retrieve_tasks_by_parent_event_id(
                event_id, app
            )
            for task in tasks:
    ...
    @staticmethod
    def retrieve_events_by_parent_id(
...
class EventsTimeSlotModelService:
    ...
    @staticmethod
    def create(
        time_slot_attrs: EventsTimeSlotsModelInterface, app: Flask, db: SQLAlchemy
    ) -> EventsTimeSlotModel:
        """
    ...
    @staticmethod
    def retrieve_slots_by_event_id(
        event_id: int, app: Flask
    ) -> List[EventsTimeSlotModel]:
        """
